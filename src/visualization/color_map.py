...
def apply_color_map(
    ...
    # Convert to NumPy so that Matplotlib color maps can be used. numpy has no bfloat16
    # equivalent, so normalize to float32 first (a no-op if x is already fp32/fp16).
...
def apply_color_map_2d(
    ...
    x_np = x.detach().float().clip(min=0, max=1).cpu().numpy()[..., None]
    y_np = y.detach().float().clip(min=0, max=1).cpu().numpy()[..., None]
