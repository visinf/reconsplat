...
from .unimatch.attention import (
...
from .unimatch.utils import (
    generate_shift_window_attn_mask,
...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        nhead=1,
        ffn_dim_expansion=4,
        wo_cross_attn=False,  # for ablation
        wo_self_attn=False,  # for ablation
    ):
        super(TransformerBlock, self).__init__()
        self.wo_cross_attn = wo_cross_attn
        self.wo_self_attn = wo_self_attn
        if (wo_self_attn is True) and (wo_cross_attn is True):
            raise Exception("should at least have one type of attention in backbone")

        if not wo_self_attn:
            self.self_attn = TransformerLayer(
                d_model=d_model,
                nhead=nhead,
                no_ffn=(not wo_cross_attn),
