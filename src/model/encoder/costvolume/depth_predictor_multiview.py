...
# avoid relying on cuSOLVER handle for generic inverse() on extrinsics
def invert_extrinsics_se3(T: torch.Tensor) -> torch.Tensor:
    """
    T: (..., 4, 4) SE(3) matrices on any device/dtype
    returns: (..., 4, 4) inverse
    """
...
def prepare_feat_proj_data_lists(
    ...
    if nn_matrix is not None:
        ...
        for idx in range(1, nn_v):
            ...
            cur_ref_pose_to_v0s = invert_extrinsics_se3(cur_view_extr) @ extrinsics
            ...
            if not (v == 2 and legacy_2views):
                ...
                for v0, v1 in zip(init_view_order, cur_view_order):
                    cur_ref_pose_to_v0_list.append(
                        ...
                        invert_extrinsics_se3(extrinsics[:, v1].clone().detach())
        ...
        if v == 2 and legacy_2views:
            ...
            pose = invert_extrinsics_se3(pose_tgt) @ pose_ref
            pose_curr_lists = [
                # torch.cat((pose, pose.inverse()), dim=0),
                torch.cat((pose, invert_extrinsics_se3(pose)), dim=0)
            ]
