...
from .backbone import (
    BackboneMultiview, BackboneUniMatch
)
...
class EncoderCostVolume(Encoder[EncoderCostVolumeCfg]):
    ...
    def __init__(self, cfg: EncoderCostVolumeCfg) -> None:
        ...
        num_views = get_cfg().dataset.view_sampler.num_context_views if self.cfg.num_context_views is None else self.cfg.num_context_views
    ...
    def forward(
        ...
        diffusion_depths: Optional[torch.Tensor] = None
    ) -> Gaussians:
        ...
        depths = diffusion_depths.view((b, v, -1, 1, 1)) if diffusion_depths is not None else depths
        ...
        return Gaussians(
            ...
            color_feature_harmonics=(
                None
                if gaussians.color_feature_harmonics is None
                else rearrange(
                    gaussians.color_feature_harmonics,
                    "b v r srf spp c d_f_sh -> b (v r srf spp) c d_f_sh",
                )
            ),
            color_features=(
                None
                if gaussians.color_features is None
                else rearrange(
            ...
            geometry_features=(
                None 
                if gaussians.geometry_features is None
                else rearrange(
                    gaussians.geometry_features, 
                    "b v r srf spp c -> b (v r srf spp) c"
                )
            )
        )
