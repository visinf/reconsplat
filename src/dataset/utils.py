...
class ScaleShiftDepthNormalizer(DepthNormalizerBase):
    ...
    def __init__(
        self, norm_min=-1.0, norm_max=1.0, min_max_quantile=0.02, clip=True, max_views=100
    ) -> None:
        self.norm_min = norm_min
        self.norm_max = norm_max
    ...
    def __call__(self, depth_linear, valid_mask=None, clip=None):
        ...
        V, _, _ = depth_linear.shape
        # If we have more than 100 views, uniform subsample along the view axis
        # This only makes sure that we can compute quantiles in a reasonable time, and because 
        # torch.quantile does not support an arbitrary number of views, due to some overflow issues.
        if V > self.max_views:
            stride = V / self.max_views
            idx = (torch.arange(self.max_views, device=depth_linear.device) * stride).long()
            view_mask = torch.zeros(V, device=depth_linear.device, dtype=torch.bool)
            view_mask[idx] = True                      

            # broadcast to (V, H, W) so it can be AND‑ed with valid_mask
            view_mask = view_mask[:, None, None]        # -> (V, 1, 1)
            valid_mask &= view_mask                     # combine in‑place

        valid_mask &= (depth_linear > 0)
