...
class ViewSamplerBoundedV2(ViewSampler[ViewSamplerBoundedV2Cfg]):
    ...
    def sample(
        ...
        if self.stage == "test":
            ...
            if sample_sequence:
                # The sequence has to fit inside [index_target_left, index_target_right], so what
                # matters is that window's LENGTH. The previous check compared the required span
                # against index_target_right alone -- i.e. as if the window started at 0 -- so a
                # short window sitting at a large offset passed the check and torch.randint was
                # called with low == high. That happens on DL3DV, where max_distance_to_context_views
                # is 0 and the window is exactly the context gap, which during warm-up can be
                # narrower than stride * num_target_views.
                window = index_target_right - index_target_left
                span = sample_sequence_stride * self.cfg.num_target_views
                if span > window:
                    # Shrink the stride to the widest one that still fits the window.
                    sample_sequence_stride = max(1, window // self.cfg.num_target_views)
                    span = sample_sequence_stride * self.cfg.num_target_views

                seq_target_left = torch.randint(
                    index_target_left,
                    # Guarantee low < high even when the window cannot hold the sequence at stride 1.
                    max(index_target_right - span + 1, index_target_left + 1),
                    size=(1,)
                ).item()
                seq_target_right = seq_target_left + span
                index_target = torch.arange(
                    seq_target_left,
                    seq_target_right,
                    step=sample_sequence_stride,
                    device=device
                )
                # arange always yields exactly num_target_views entries (so batches stay collatable);
                # clamp in case a too-narrow window pushed the tail past the last real frame.
                index_target = index_target.clamp(max=num_views - 1)
                
            # When training or validating (visualizing), pick at random.
            elif self.cfg.target_views_replace_sample:
        ...
        # print(f"sampling...(context and target) [sample_sequence = {sample_sequence}]")
        # print((index_context_left, *extra_views, index_context_right))
        # print(index_target)
